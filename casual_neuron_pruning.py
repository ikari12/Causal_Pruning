...
def report_sparsity(model: nn.Module, description: str):
    ...
    print("-" * 40)
    # ▼▼▼ CHANGE 1: Return the calculated sparsity value ▼▼▼
    return sparsity
...
def prune_model_with_causal_scores(model: nn.Module, summary_df: pd.DataFrame, sparsity: float):
    ...
    """
    if not 0 <= sparsity < 1:
        print(f"⚠️ Sparsity must be between 0 and 1. Got {sparsity}. Skipping pruning.")
        return model
    
    if sparsity == 0:
        print("✅ Sparsity is 0, skipping pruning.")
        return model

    print(f"✂️ Starting pruning for {sparsity*100:.1f}% sparsity...")
...
def evaluate_model(model_to_eval, tokenizer_to_use, sparsity_level):
    ...
    """
    print(f"\n--- 🔬 Starting Performance Evaluation on JSTS for {sparsity_level*100:.0f}% sparsity ---")
...
if __name__ == "__main__":
    ...
    for sparsity in sparsity_levels:
        ...
        # ▼▼▼ CHANGE 2: Capture the returned value and add it to the results list ▼▼▼
